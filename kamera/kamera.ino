...
void setup()
{
  ...
  u8x8log.setRedrawMode(1);		// 0: Update screen with newline, 1: Update screen for every char 
    while (!Serial){
      u8x8log.print("Czekam..\n");
      delay(500);
      }
    Serial.println("Edge Impulse Inferencing Demo");
    u8x8log.print("Leszek \nMechatronics\n");
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        if (ei_camera_capture(EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, snapshot_buf) == false) {
            ...
            u8x8log.print("Failed to capture image\r\n");
        ...
        if (ei_error != EI_IMPULSE_OK) {
            ...
            u8x8log.print("Failed to run impulse\n");
        ...
        for (size_t ix = 0; ix < result.bounding_boxes_count; ix++) {
            ...
            u8x8log.print(bb.label);
            u8x8log.print( "\n");
            u8x8log.print( bb.value);
            u8x8log.print( "\n");
